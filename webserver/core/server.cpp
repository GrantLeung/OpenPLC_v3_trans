...
bool SetSocketBlockingEnabled(int fd, bool blocking)
{
   ...
   int flags = ioctlsocket(fd, FIONBIO, 0);
   //int flags = ioctlsocket(fd, FIONBIO, 0);
   //change the fcntl to the ioctlsocket,by lianggh
   if (flags == -1) return false;
   flags = blocking ? (flags & ~non_blk) : (flags | non_blk);

   //10-18 revised
   //return (fcntl(fd, F_SETFL, flags) == 0) ? true : false;
   return (ioctlsocket(fd, FIONBIO, flags) == 0) ? true : false;
   //change the fcntl to the ioctlsocket,by lianggh
}
